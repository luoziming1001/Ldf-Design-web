...
export default function Works() {
  ...
  // Load custom data from IndexedDB on mount with localStorage as fallback
  useEffect(() => {
    async function loadStoredData() {
      let savedCardCustom = await idbGet<Record<string, string>>("cardCustomImages");
      let savedProjectGallery = await idbGet<Record<string, Record<number, string>>>("projectCustomGallery");

      if (!savedCardCustom) {
        try {
          const local = localStorage.getItem("cardCustomImages");
          savedCardCustom = local ? JSON.parse(local) : {};
        } catch {
          savedCardCustom = {};
        }
      }
      if (!savedProjectGallery) {
        try {
          const local = localStorage.getItem("projectCustomGallery");
          savedProjectGallery = local ? JSON.parse(local) : {};
        } catch {
          savedProjectGallery = {};
        }
      }

      setCardCustomImages(savedCardCustom || {});
      setProjectCustomGallery(savedProjectGallery || {});
      isStorageInitialized.current = true;
    }
    loadStoredData();
  }, []);

  // Save to IndexedDB (and fallback localStorage) on changes, but only after initialization
  useEffect(() => {
    if (!isStorageInitialized.current) return;
    async function saveCardCustom() {
      await idbSet("cardCustomImages", cardCustomImages);
      try {
        localStorage.setItem("cardCustomImages", JSON.stringify(cardCustomImages));
      } catch (e) {
        console.warn("localStorage full, saved to IndexedDB only:", e);
      }
    }
    saveCardCustom();
  }, [cardCustomImages]);
  ...
  useEffect(() => {
    if (!isStorageInitialized.current) return;
    async function saveProjectGallery() {
      await idbSet("projectCustomGallery", projectCustomGallery);
      try {
                          ...
                          if (files && files.length > 0) {
                            ...
                            Promise.all(promises).then((results) => {
                              ...
                              if (validResults.length > 0) {
                                setProjectCustomGallery((prev) => {
                                  const newProjectCustoms: Record<number, string> = {};
                                  validResults.forEach((base64Src, index) => {
                                    newProjectCustoms[index] = base64Src;
                                  });
                                  return {
                                    ...prev,
                                    [selectedProject.id]: newProjectCustoms
                                  };
                                });
                                setActiveImageIndex(0);
                              }
